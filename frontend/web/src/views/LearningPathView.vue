...
<template>
  <div class="page-stack">
    ...
    <template v-else>
      <section class="path-banner"><div class="path-banner-icon"><Network :size="24" /></div><div><span class="eyebrow">CURRENT RECOMMENDATION</span><h2>{{ path.currentNode?.title || path.currentNode?.name || "正在规划你的下一步" }}</h2><p>{{ path.currentNode?.summary || "系统正在根据你的掌握度和先修条件选择当前推荐节点。" }}</p></div><span class="status-pill status-pill-success">{{ path.run?.status || "规划中" }}</span></section>
