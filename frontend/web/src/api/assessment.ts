...
export const assessmentApi = {
  ...
  practiceReview: (runId: string, payload: Record<string, unknown>) =>
    api.post(`/api/v1/runs/${encodeURIComponent(runId)}/practice-review`, payload, { timeout: RESOURCE_GENERATION_TIMEOUT_MS }).then((r) => r.data),
  coach: (runId: string, payload: Record<string, unknown>) =>
    api.post(`/api/v1/runs/${encodeURIComponent(runId)}/coach`, payload, { timeout: RESOURCE_GENERATION_TIMEOUT_MS }).then((r) => r.data),
