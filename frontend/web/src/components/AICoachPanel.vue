...
function send() { if (message.value.trim()) { emit("send", message.value.trim()); message.value = ""; } }
</script>
<template>
  <section class="coach-panel" :class="{ compact }">
    ...
    <div class="coach-input"><input v-model="message" placeholder="向 AI 学习顾问提问" @keyup.enter="send" /><button class="icon-button" title="发送问题" @click="send"><Send :size="16" /></button></div>
  </section>
</template>
