...
# 评价标准 (供用户选择)
LEVEL_OPTIONS = ["未学过", "入门", "基础", "熟练", "精通"]


class CourseSelfAssessment(BaseModel):
    """细分课程自评 — 某个领域下的具体课程评价"""
    name: str = Field("", description="课程名")
    level: str = Field("未学过", description="评价标准: 未学过/入门/基础/熟练/精通")
...
class DomainAssessment(BaseModel):
    ...
    courses: List[CourseSelfAssessment] = Field(default_factory=list, description="细分课程自评")
    note: str = Field("", description="该领域自评填空")
