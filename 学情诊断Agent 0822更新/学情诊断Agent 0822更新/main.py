...
class AdaptiveTestConfigRequest(BaseModel):
    domains: list[str] | None = None
    knowledge_point_ids: list[str] | None = None
    max_questions_per_domain: int = Field(20, ge=1, le=20)
    stop_consecutive_wrong: int = Field(3, ge=1, le=10)
...
async def start_adaptive_test(learner_id: str, payload: Optional[AdaptiveTestConfigRequest] = Body(default=None)):
    """启动自适应测试 — 返回第一道题
    ...
    try:
        config = adaptive_test.build_config(payload.model_dump(exclude_none=True) if payload else None)
