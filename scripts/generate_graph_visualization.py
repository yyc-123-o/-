...
function select(id) {{ state.selectedId=id; const item=byId.get(id); if(item.type==='chapter') state.chapterId=state.chapterId===id?null:id; render(); const related=DATA.edges.filter(edge=>edge.source===id||edge.target===id); const resources=resourcesByConcept.get(id)||[]; const resourceHtml=item.type==='concept' ? `<p>候选资源绑定：${{resources.length}} 条</p>` + (resources.length ? `<div>${{resources.slice(0,8).map(resource=>`<span class="pill">${{resource.source_title}} · ${{resource.match_type}}</span>`).join('')}}${{resources.length>8?`<span class="pill">还有 ${{resources.length-8}} 条</span>`:''}}</div><p class="warn">候选状态：未审核，不能作为正式证据</p>` : '<p class="empty">暂无候选资源绑定</p>') : ''; document.getElementById('selected').innerHTML=`<h2>${{item.label}}</h2><p><code>${{item.id}}</code></p><p>${{item.summary||item.labelEn||''}}</p><p>${{related.length}} 条关联边</p>${{item.type==='concept'?`<p>章节：${{chapterById.get(item.chapterId).label}}</p>${{resourceHtml}}`:''}}`; }}
