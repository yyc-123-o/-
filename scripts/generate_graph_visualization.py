...
def graph_payload(catalog: OntologyCatalog) -> dict[str, Any]:
...
<style>
:root {{ color-scheme: light; --ink:#17212b; --muted:#637083; --line:#d7dee7; --panel:#ffffff; --bg:#f4f7fa; --blue:#1f6feb; --blue-soft:#e7f0ff; --amber:#b46900; --amber-soft:#fff4d6; --red:#b42318; --red-soft:#fff0ee; --green:#16805d; --green-soft:#e5f6ef; }}
...
.node {{ cursor:pointer; stroke-width:1.3; }} .node.chapter {{ fill:var(--blue); stroke:#124f9e; }} .node.section {{ fill:var(--amber-soft); stroke:#d9a13a; }} .node.concept {{ fill:var(--green-soft); stroke:#65ad91; }} .node.selected {{ stroke:#17212b; stroke-width:3; }} .node.dim {{ opacity:.18; }} .label {{ font-size:10px; pointer-events:none; fill:var(--ink); }} .label.chapter {{ fill:white; font-weight:600; font-size:11px; }} .label.dim {{ opacity:.2; }}
...
<body>
<header><h1>AI Learning Course Graph</h1><div class="subtitle">ai-course-v1 · 140 个概念 · 11 个章节 · 147 条关系 · 点击章节或节点检查课程逻辑</div></header>
<main>
<aside><div class="section-title">章节导航</div><div id="chapters" class="chapter-list"></div><div class="stats"><div class="stat"><strong id="conceptCount"></strong><span>概念节点</span></div><div class="stat"><strong id="edgeCount"></strong><span>先修关系</span></div><div class="stat"><strong id="crossCount"></strong><span>跨章节先修</span></div><div class="stat"><strong id="cycleCount"></strong><span>检测环路</span></div></div><div class="section-title">关系图例</div><div class="legend"><span><i style="background:#6683a5"></i>硬先修</span><span><i style="background:#b98b39"></i>软先修</span><span><i style="background:#c28787"></i>对比/易混淆</span></div></aside>
<section class="canvas"><div class="toolbar"><strong id="viewTitle">课程章节主干</strong><span class="legend"><span><i style="background:#1f6feb"></i>章节</span><span><i style="background:#d9a13a"></i>小节</span><span><i style="background:#65ad91"></i>概念</span></span></div><svg id="graph" role="img" aria-label="课程知识图谱关系图"><defs><marker id="arrow" markerWidth="8" markerHeight="8" refX="7" refY="3" orient="auto"><path d="M0,0 L8,3 L0,6 Z" fill="#6683a5"></path></marker></defs><g id="edges"></g><g id="nodes"></g></svg></section>
...
<script>
const DATA = {data};
const state = {{ chapterId: null, selectedId: null }};
...
function render() {{
  ...
  [...DATA.chapters,...DATA.sections,...DATA.concepts].filter(visible).forEach(item=>{{ const p=position.get(item.id); const group=document.createElementNS('http://www.w3.org/2000/svg','g'); const node=document.createElementNS('http://www.w3.org/2000/svg','rect'); node.setAttribute('x',p.x-(item.type==='chapter'?52: item.type==='section'?38:30)); node.setAttribute('y',p.y-(item.type==='chapter'?18:14)); node.setAttribute('width',item.type==='chapter'?104:item.type==='section'?76:60); node.setAttribute('height',item.type==='chapter'?36:28); node.setAttribute('rx','4'); node.setAttribute('class',`node ${{item.type}} ${{state.selectedId===item.id?'selected':''}}`); node.addEventListener('click',()=>select(item.id)); group.appendChild(node); const text=document.createElementNS('http://www.w3.org/2000/svg','text'); text.setAttribute('x',p.x); text.setAttribute('y',p.y+4); text.setAttribute('text-anchor','middle'); text.setAttribute('class',`label ${{item.type}}`); text.textContent=item.type==='chapter'?`第${{item.order}}章`:item.label.length>7?item.label.slice(0,7)+'…':item.label; group.appendChild(text); nodesEl.appendChild(group); }});
  document.getElementById('viewTitle').textContent = state.chapterId ? `${{chapterById.get(state.chapterId).label}} · 概念依赖` : '课程章节主干';
}}
function select(id) {{ state.selectedId=id; const item=byId.get(id); if(item.type==='chapter') state.chapterId=state.chapterId===id?null:id; render(); const related=DATA.edges.filter(edge=>edge.source===id||edge.target===id); document.getElementById('selected').innerHTML=`<h2>${{item.label}}</h2><p><code>${{item.id}}</code></p><p>${{item.summary||item.labelEn||''}}</p><p>${{related.length}} 条关联边</p>${{item.type==='concept'?`<p>章节：${{chapterById.get(item.chapterId).label}}</p>`:''}}`; }}
