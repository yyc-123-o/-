...
def _resource_items(resource_type: ResourceType, brief: ResourceBrief) -> tuple[str, ...]:
    ...
    personalization = _personalization_items(brief, topic)
    if resource_type is ResourceType.LECTURE:
        return (
            *personalization,
            f"学习目标：{outcome}。本节围绕“{topic}”建立从输入、变换到输出的完整解释链。",
            f"核心定义：先明确“{topic}”接收的输入表示、允许的操作和输出含义；任何公式都必须同时满足这些边界条件。",
            "推导与示例：选择最小数值例子，逐步写出中间量，再用代码打印输入、关键参数和结果，核对手算与实现是否一致。",
            "边界与辨析：比较一个相邻概念或参数变化，说明结果为什么不同；重点检查形状、顺序、步长和边界处理。",
    ...
    if resource_type is ResourceType.ASSESSMENT:
        return (
            *personalization,
            ...
            "形状推理：修改一个关键参数，重新推导输出形状，说明变化是由哪条规则导致的。",
...
def _personalization_items(brief: ResourceBrief, topic: str) -> tuple[str, ...]:
    """Materialize the learner policy in formal resources.

    The formal generator is deterministic, but its first item explicitly
    reflects the same adaptation facts used by planning. This prevents strict
    resources from collapsing to one template for every learner.
    """
    adaptation = brief.node_adaptation
    allocation = brief.resource_allocation
    preferences = brief.presentation_preferences
    support = adaptation.support_intensity.value
    if support in {"remediation", "scaffolded"}:
        lead = f"个性化支架（{support}）：围绕“{topic}”先给出分步提示，再逐步减少提示。"
    elif support == "compact":
        lead = f"个性化路径（{support}）：已具备“{topic}”基础，压缩重复定义，直接进入关键辨析。"
    else:
        lead = f"个性化路径（{support}）：用一个最小例子串联“{topic}”的概念、推导和应用。"
    items = [lead]
    if preferences.content_order:
        items.append("讲解顺序：" + " → ".join(preferences.content_order) + "。")
    if brief.error_pattern_hints:
        codes = "、".join(item.code for item in brief.error_pattern_hints)
        items.append(f"错因针对：本学习者在“{topic}”相关内容中的重点错因是{codes}，增加对比和反例。")
    items.append(
        f"资源配额：预计{allocation.estimated_minutes}分钟，安排{allocation.worked_example_count}个示例、"
        f"{allocation.guided_exercise_count}个引导练习。"
    )
    return tuple(items)
